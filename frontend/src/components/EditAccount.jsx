...
export const EditAccount = ({isOpen, setIsOpen, userName, mandarinName, email, dateOfBirth, gender, address, phoneNumber, institution, setUsername, setMandarinName, setEmail, setDateOfBirth, setGender, setAddress, setPhoneNumber, setInstitution}) => {
    ...
    return(
        <Modal isOpen={isOpen} onRequestClose={() => {
            setIsOpen(false);
        }} className="items-center font-poppins flex flex-col w-[90%] md:w-[70%] bg-white mx-auto shadow-xl relative overflow-y-auto scrollbar-thin rounded-[10px] xl:max-w-[1200px] 2xl:max-w-[1800px]" overlayClassName="flex justify-center items-center inset-0 fixed z-1000 bg-[rgba(0,0,0,0.5)] overflow-hidden">
            ...
            <div className="grid grid-cols-[auto] max-h-[60vh] lg:max-h-none overflow-y-auto sm:grid-cols-[auto_1fr] gap-y-[0.5em] md:gap-y-[1.2em] gap-x-[1.5em] mt-[2em]">
                <p>Name:</p>
             ...
             <button className='color-component-red rounded-md hover:!bg-red-700 duration-300 text-white cursor-pointer py-2 px-3 mt-[2em] mb-[2em]' onClick={() => {applyChanges()}}>Save Changes</button>
        </Modal>
    );
}